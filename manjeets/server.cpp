...
void block_sender(char *sender_ipadr, char *user_toblock_ip) {
	...
	sender_addr = (char *)malloc(sizeof (char)*(len));
	sender_addr[len] = '\0';
	...
	rcvr_blockaddr = (char *)malloc(sizeof (char)*(len));
	rcvr_blockaddr[len] = '\0';
		...
		if (strncmp(ip_sender, itr->user_adderss, strlen(ip_sender)) != 0) {
			...
			if (findBlockedUser(ip_sender, addrs_rcvr) < 0) {
				...
				if (isClientLoggedOut(recvrfd) == 0) {
					...
					strncat(msg_for_recvr, "255.255.255.255", strlen("255.255.255.255"));
					strcat(msg_for_recvr, "-");
...
void create_command_token(char *buff) {
	...
	tokens = strtok(buff, " ");
	while (tokens != NULL) {
		temp_msg = (char *) malloc(sizeof(char) * (incr));
		...
		if (issendmsg == 0 && incr == 1) {
			...
			msg_arr = (char *) malloc(sizeof(char) * (strlen(temp_msg)));
