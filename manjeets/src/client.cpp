...
void block_user(int listener_client, char *to_block_ip) {
	char *blckbuffer;
	int len = strlen(to_block_ip)+strlen(client_ip)+5;
	blckbuffer = (char *)malloc(sizeof(char)*(len));
	blckbuffer[len]= '\0';
	strcpy(blckbuffer, "UB");
	strcat(blckbuffer, " ");
	strncpy(blckbuffer, client_ip, strlen(client_ip));
	strcat(blckbuffer, " ");
	strncat(blckbuffer, to_block_ip, strlen(to_block_ip));
	sendall(listener_client,blckbuffer,&len);
...
void unblock_user(int listener_client, char *to_block_ip) {
	...
	strcat(blckbuffer, " ");
	strncat(blckbuffer, to_block_ip, strlen(to_block_ip));
	int bytes_sent = sendall(listener_client,blckbuffer,&len);
	...
	for (j = 1, str1 = list_arr; ; j++, str1 = NULL) {
	   ...
	   for (str2 = token; ; str2 = NULL) {
		   ...
		   if(i == 0) {
			   ...
			   char *nme = (char *)malloc(sizeof (char)*n+1);
			   nme[n] = '\0';
			   memcpy(nme, subtoken, n);
			   tempObj.machine_name = nme;
		   } else if(i == 1) {
			   int n = strlen(subtoken);
			   char *addrs = (char *)malloc(sizeof (char)*n+1);
			   addrs[n] = '\0';
			   memcpy(addrs, subtoken, n);
			   tempObj.user_adderss = addrs;
	       } else if(i == 2) {
			   int n = strlen(subtoken);
			   char *portchar = (char *)malloc(sizeof (char)*n)+1;
			   portchar[n] = '\0';
			...
			if (FD_ISSET(k, &read_fdsclient)) { // we got one!  - referred from Beej guide
				if(FD_ISSET(STDIN_FILENO, &read_fdsclient)){
					...
					while(token_client != NULL) {
						...
						*/
						if(issendcmd == 0 && count == 1) {
							...
							server_ipaddress = (char *)malloc(sizeof(char)*(strlen(temp_char)+1));
							server_ipaddress[strlen(temp_char)] = '\0';
							strncat(server_ipaddress, temp_char, strlen(temp_char));
						}
						count++;
					}
					...
					if (nbytes_client <= 0) {
						...
						read_buffer_client[nbytes_client] = '\0';
